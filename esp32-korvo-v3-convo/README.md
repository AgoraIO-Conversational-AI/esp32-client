...
Before building the project, you need to configure your API keys and credentials in `main/app_config.h`:
...
2. Edit `main/app_config.h` and replace the following placeholders with your actual credentials:
   - `YOUR_AGORA_API_KEY` - Your Agora API Key
   - `YOUR_AGORA_API_SECRET` - Your Agora API Secret
   - `YOUR_AGORA_APP_ID` - Your Agora App ID
   - `YOUR_OPENAI_API_KEY` - Your OpenAI API Key
   - `YOUR_AZURE_TTS_API_KEY` - Your Azure TTS API Key
   - `YOUR_AZURE_REGION` - Your Azure region (e.g., "eastus")

3. Configure other settings as needed:
   - `AI_AGENT_NAME` - Name of your AI agent
   - `AI_AGENT_CHANNEL_NAME` - RTC channel name
   - `AI_AGENT_TOKEN` - RTC token for AI agent (leave empty if not using token authentication)
   - `AGORA_RTC_TOKEN` - RTC token for board (leave empty if not using token authentication)

**⚠️ Important:** Never commit your `main/app_config.h` file with real credentials to version control. The file is included in `.gitignore` to prevent accidental commits.
